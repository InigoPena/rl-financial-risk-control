...
# ---------------------------------------------------------------------
# Callback to sync VecNormalize stats (train -> eval)
# ---------------------------------------------------------------------
class SyncVecNormalizeEvalCallback(EvalCallback):
    """
    Ensures eval env uses the SAME normalization statistics as train env.
    This is important when using VecNormalize and evaluating on a separate env.
    """
    def __init__(self, train_norm_env: VecNormalize, eval_norm_env: VecNormalize, *args, **kwargs):
        super().__init__(eval_norm_env, *args, **kwargs)
        self.train_norm_env = train_norm_env
        self.eval_norm_env = eval_norm_env

    def _on_step(self) -> bool:
        # Before running an evaluation, EvalCallback checks eval_freq inside _on_step
        # We sync stats every step; cheap and robust.
        if isinstance(self.train_norm_env, VecNormalize) and isinstance(self.eval_norm_env, VecNormalize):
            self.eval_norm_env.obs_rms = self.train_norm_env.obs_rms
            # If you ever enable reward normalization in eval (not recommended), also sync ret_rms:
            # self.eval_norm_env.ret_rms = self.train_norm_env.ret_rms
...
def main():
    ...
    # Training env (VecNormalize learns stats here)
    ...
    # Eval env (uses same split as your test.py)
    ...
    eval_vec_env = make_vec_env(GoldHedgeEnv, n_envs=1, env_kwargs=eval_env_kwargs, seed=SEED)
    # IMPORTANT: eval env must NOT update stats, and reward should not be normalized for interpretability
    eval_vec_env = VecNormalize(eval_vec_env, norm_obs=True, norm_reward=False, training=False, clip_obs=10.)
    ...
    # Slightly more conservative LR schedule for stability
    ...
    model = PPO(
        ...
        n_steps=1024,          # more stable gradients in finance
        batch_size=512,
        n_epochs=10,
        gamma=0.995,
        gae_lambda=0.95,
    ...
    eval_callback = SyncVecNormalizeEvalCallback(
        train_norm_env=train_vec_env,
        eval_norm_env=eval_vec_env,
    ...
    # Save VecNormalize stats for test.py
    train_vec_env.save(os.path.join(current_dir, "vec_normalize.pkl"))

    print(f"Final model saved to {model_save_path}")
    print("Saved VecNormalize stats to vec_normalize.pkl")
