"""
SafePolicy-RM: Integrated Risk Management Agent for USD Trading
Data Download and Feature Engineering Script

This script downloads USD index data from Yahoo Finance (2015-present)
...
def download_and_process_usd_data(start_date='2015-01-01', end_date=None):
    """
    Download USD data and calculate all indicators
    ...
    print(f"Downloading USD Index (DX-Y.NYB) data from {start_date} to {end_date}...")
    
    # Download USD Index (DX-Y.NYB = U.S. Dollar Index)
    # Alternative: UUP (Invesco DB USD Index Bullish Fund ETF)
    usd = yf.download('DX-Y.NYB', start=start_date, end=end_date, progress=False)
    
    if usd.empty:
        print("No data found for DX-Y.NYB, trying UUP (USD ETF)...")
        usd = yf.download('UUP', start=start_date, end=end_date, progress=False)
    
    print(f"Downloaded {len(usd)} rows of data")
    
    # Create feature dataframe
    df = pd.DataFrame(index=usd.index)
    df['Close'] = usd['Close']
    df['High'] = usd['High']
    df['Low'] = usd['Low']
    df['Open'] = usd['Open']
    df['Volume'] = usd['Volume']
...
def save_data(df, filename='usd_data_safepolicy.parquet'):
...
def display_summary(df):
    ...
    print("\n" + "="*70)
    print("SUMMARY STATISTICS - USD INDEX")
    print("="*70)
    ...
    print("\n💵 PRICE CONTEXT:")
    print(f"  USD Index (latest):       {df['Close'].iloc[-1]:.2f}")
    print(f"  USD Index (min):          {df['Close'].min():.2f}")
    print(f"  USD Index (max):          {df['Close'].max():.2f}")
...
if __name__ == "__main__":
    print("="*70)
    print("SafePolicy-RM: USD Trading Risk Management Agent")
    print("Data Preparation Script")
    print("="*70)
    
    # Download and process data
    df = download_and_process_usd_data(start_date='2015-01-01')
    
    # Display summary
    display_summary(df)
    
    # Save to Parquet
    save_data(df, 'usd_data_safepolicy.parquet')
    ...
    print("\nNext steps:")
    print("  1. Load 'usd_data_safepolicy.parquet' in your gym-trading environment")
